...
let bot = {
  move: function() {
    let keyEventCodeLEFT = (ball.y < platformLEFT.y + platformLEFT.h/2)? 87: 83;
    let keyEventCodeRIGHT = (ball.y < platformRIGHT.y + platformRIGHT.h/2)? 73: 75;

    if(ball.xVel / Math.abs(ball.xVel) == -1) keyEvent(keyEventCodeLEFT, 5);
    if(ball.xVel / Math.abs(ball.xVel) == 1) keyEvent(keyEventCodeRIGHT, 5);
  }
};
